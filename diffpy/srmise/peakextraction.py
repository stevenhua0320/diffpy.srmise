...
class PeakExtraction(object):
    ...
    def __init__(self, newvars=[]):
        ...
        self.clear()
        self.extractvars = dict.fromkeys(('effective_dy', 'rng', 'pf', 'initial_peaks', 'baseline', 'cres', 'error_method'))
        for k in newvars:
    ...
    def extract_single(self, recursion_depth=1):
        ...
        for step in dclusters:
            ...
            if len(step.clusters) > len(mclusters):
                # Add a new cluster
                mclusters.insert(step.lastcluster_idx,
                                 ModelCluster(None,
                                              bl,
                                              x,
                                              y,
            ...
            if step.lastpoint_idx == 0 or step.lastpoint_idx == len(step.x)-1:
                ...
                adj_cluster = ModelCluster(near_peaks, bl, adj_x, adj_y, adj_error, slice(len(adj_x)), self.error_method, self.pf)
                ...
                checkrec = ModelCluster(None, None, rec_r, rec_y, rec_error, None, self.error_method, self.pf)
                recurse = len(near_peaks) > 0 and checkrec.quality().growth_justified(checkrec, min_npars)

                if recurse and recursion_depth < 3:
                    logger.info("\n*********STARTING RECURSION level %s (full boundary)************" %(recursion_depth+1))
                    rec_search = PeakExtraction()
                    rec_search.setdata(rec_r, rec_y, None, rec_error)
                    rec_search.setvars(quiet=True, baseline=bl, cres=self.cres, pf=self.pf, error_method=self.error_method)
            ...
            for idx in adjacent[-1:0:-1]:
                ...
                if len(near_peaks) == 2:
                    ...
                    if recurse1 and recursion_depth < 3:
                        logger.info("\n*********STARTING RECURSION level %s (reduce at border)************" %(recursion_depth+1))
                        rec_search1 = PeakExtraction()
                        rec_search1.setdata(rec_r1, rec_y1, None, rec_error1)
                        rec_search1.setvars(quiet=True, baseline=bl, cres=self.cres, pf=self.pf, error_method=self.error_method)
                        rec_search1.extract_single(recursion_depth+1)
                        rec1 = rec_search1.extracted
                        logger.info("*********ENDING RECURSION level %s (reduce at border) ************\n" %(recursion_depth+1))
                ...
                # Quick check to see if there is anything to find
                min_npars = min([p.npars for p in self.pf])
                checkrec = ModelCluster(None, None, rec_r2, rec_y2, rec_error2, None, self.error_method, self.pf)
                ...
                if recurse2 and recursion_depth < 3:
                    ...
                    rec_search2.setvars(quiet=True, baseline=bl, cres=self.cres, pf=self.pf, error_method=self.error_method)
