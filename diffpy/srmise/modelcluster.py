    ...
    def transform(self, in_format, out_format, **kwds):
        ...
        for i in parts:
            ...
            try:
                ...
                logger.warning("Transformation gradient not implemented for part %i: %s.  Ignoring transformation." %(i, str(p)))
            ...
            try:
                ...
                logger.warning("Parameter transformation failed for part %i: %s.  Failed with message %s. Ignoring transformation." %(i, str(p), str(e)))
                subg = np.identity(p.npars(True))
...
class ModelCluster(object):
    ...
    def join_adjacent(m1, m2):
        ...
        if len(m1.model) > 0 and len(m2.model) > 0:
            ...
            for i in reversed(range(len(new_model))):
                if new_model[i]["position"] == border_x and \
                   i > 0 and new_model[i-1]["position"] == border_x:
                    del new_model[i]
                elif new_ids[i] != i:
        ...
        return ModelCluster(new_model, m1.baseline, m1.r_data, m1.y_data,
                            m1.y_error, new_slice, m1.error_method, peak_funcs)
    ...
    def fit(self, justify=False, ntrials=0, fitbaseline=False, estimate=True, cov=None, cov_format="default_output"):
        ...
        if new_qual < orig_qual:
            ...
            msg = ["ModelCluster.fit() warning: fit seems not to have improved.",
    ...
    def contingent_fit(self, minpoints, growth_threshold):
        ...
        if self.never_fit:
            return None
        if (self.last_fit_size > 0 and float(self.size)/self.last_fit_size >= growth_threshold) \
